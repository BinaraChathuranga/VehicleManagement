...
@section('content')   
  ...
  <div class="col-lg-12 col-12">
   ...
   <div class="btn-group btn-group-toggle" data-toggle="buttons">
    ...
    <a href="/zonalAdmin/vehicleDetailsReport" name="options" id="option1" class="btn btn-dark">Vehicle Report</a>
    <a href="/zonalAdmin/viewAllReservations" name="options" id="option1" class="btn btn-dark">Reservation Report</a>

    @if(Illuminate\Support\Facades\DB::table('fuelconsuption')->where('vehicleNo','=',$dvInfo->regNo)->count() > 0)
    <a href="/zonalAdmin/vehFuelConsumption/{{$dvInfo->regNo}}" name="options" id="option1" class="btn btn-dark">Fuel Consumption Report</a>
    <a href="/zonalAdmin/viewRefillDetails/{{$dvInfo->regNo}}" name="options" id="option1" class="btn btn-dark">Fuel Refilled Report</a>
    <a href="/zonalAdmin/runnigChart/{{$dvInfo->regNo}}" name="options" id="option1" class="btn btn-dark">Runing Chart</a>

    @elseif(Illuminate\Support\Facades\DB::table('fuelconsuption')->where('vehicleNo','=',$dvInfo->regNo)->count() == 0)
    <a href="/zonalAdmin/vehFuelConsumption/{{$dvInfo->regNo}}" hidden name="options" id="option1" class="btn btn-dark">Fuel Consumption Report</a>
    <a href="/zonalAdmin/viewRefillDetails/{{$dvInfo->regNo}}" hidden name="options" id="option1" class="btn btn-dark">Fuel Refilled Report</a>
    <a href="/zonalAdmin/runnigChart/{{$dvInfo->regNo}}" hidden name="options" id="option1" class="btn btn-dark">Runing Chart</a>
    @endif

    @if(Illuminate\Support\Facades\DB::table('servicedetails')->where('vehicleNoService','=',$dvInfo->regNo)->count() > 0)
    <a href="/zonalAdmin/viewServiceDetails/{{$dvInfo->regNo}}" name="options" id="option1" class="btn btn-dark">Service Report</a>
    @elseif(Illuminate\Support\Facades\DB::table('servicedetails')->where('vehicleNoService','=',$dvInfo->regNo)->count() == 0)
    <a href="/zonalAdmin/viewServiceDetails/{{$dvInfo->regNo}}" hidden name="options" id="option1" class="btn btn-dark">Service Report</a>
    @endif

    @if(Illuminate\Support\Facades\DB::table('repairDetails')->where('vehicleNoRepair','=',$dvInfo->regNo)->count() > 0)
    <a href="/zonalAdmin/viewRepairDetails/{{$dvInfo->regNo}}" name="options" id="option1" class="btn btn-dark">Repair Report</a>
    @elseif(Illuminate\Support\Facades\DB::table('repairDetails')->where('vehicleNoRepair','=',$dvInfo->regNo)->count() == 0)
    <a href="/zonalAdmin/viewRepairDetails/{{$dvInfo->regNo}}" hidden name="options" id="option1" class="btn btn-dark">Repair Report</a>
    @endif
    </div>
  </center>
  </div>
