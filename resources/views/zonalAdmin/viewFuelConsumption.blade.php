  ...
  <!-- New driver modal -->
    <section class="content">
        ...
        <div class="card">
         ...
         <form action="/zonalAdmin/filterFuelConsumption" method="POST">
            {{csrf_field()}}

          <div class="row">
              <input type="text" name="vName" value="{{$fc->vehicleNo}}" hidden>
              <div class="col-12 col-lg-5">
          <h3 class="m-0 text-dark ml-3 mb-3"> Vehicle No : {{$fc->vehicleNo}} </h3>
          </div>
      ...
      <div id="resReport">
<?php
$zone=App\vehicleDetail::where('regNo','=',$fc->vehicleNo)->distinct('zone')->first();
?>
...
<div class="alert alert-light collapse" role="alert" style="text-align: center;" id="alert">
  ...
  <h6 style="color: black;">Vehicle No : {{$fc->vehicleNo}} | {{$zone->zone}} | All Fuel Consumptions<h6>
</div>   
    <table class="table table-striped table-bordered" id="vehicle">
      <thead>
        ...
        <tbody>
            @foreach(App\fuelConsumption::where('vehicleNo','=',$fc->vehicleNo)->where('status','=','Consumption')->get(); as $fc)
